...
- **`RithmicBracketOrder::operation_type`** and the `BracketOperationType` enum — the `order_operation_type` field Rithmic added to bracket requests in template version 5.37. Rithmic documents only the wire spellings (`AFOCCA`, `FOCCA`, `CCA`, `FCA`, `OCA`); unset, the field stays off the wire as before.
