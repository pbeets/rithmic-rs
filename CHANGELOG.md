...
- **The bundled protos are refreshed to R | Protocol API 0.89.0.0 (template version 5.42), and `RithmicMessage::AccountListUpdates` goes with them.** Rithmic removed `account_list_updates.proto` (template 354) from the proto pool — its release notes say to use the account RMS updates stream instead, so subscribe with `subscribe_account_rms_updates` and match `RithmicMessage::AccountRmsUpdates`. In the same refresh Rithmic replaced the two enum fields on `UserAccountUpdate` with strings: `update_type` and `access_type` are now `Option<String>` (spellings like `add_account`, `modify_account`), and the generated `UpdateType`/`AccessType` enums no longer exist.
...
- **`RithmicAdvancedBracketOrder` is removed; `RithmicBracketOrder` absorbed it.** One bracket type now carries every venue-native field — trailing stops, break-even, timed release/cancel, if-touched entry — and `place_advanced_bracket_order` is gone. Call `place_bracket_order`. `profit_ticks`/`stop_ticks` became `target_quantity`/`target_ticks` and `stop_quantity`/`stop_ticks`, all `Vec<i32>`, one entry per exit leg. The single-value **setters** `.profit_ticks(n)`/`.stop_ticks(n)` are renamed `.target(n)`/`.stop(n)` — the *fields* `target_ticks`/`stop_ticks` keep their names, so reading `bracket.stop_ticks` is unaffected. Singular sizes one leg to the entry quantity and reads it when called, so set `.quantity()` first — `build()` rejects the zero-sized leg the other order produces; plural `.targets(..)`/`.stops(..)` take explicit `(quantity, ticks)` pairs and have no ordering requirement. `bracket_type` is now `Option<BracketType>`: `build()` derives it from which exit legs are supplied (as async_rithmic does) and omits it from the request when there are none, where `RithmicAdvancedBracketOrder` defaulted it to `TargetAndStopStatic` and always sent it — a target-only bracket now goes out as `TARGET_ONLY_STATIC` rather than `TARGET_AND_STOP_STATIC`. Set `.bracket_type(..)` to override the derivation; `validate()` rejects one that names sides the legs do not form.
...
- **`#[non_exhaustive]` on `RithmicConfig`, `RithmicAccount`, `LoginConfig`, `InstrumentInfo`, `RithmicEnv`, `TrailingStop` and all 244 generated protobuf types.** Rithmic added fields to existing messages in 24 of the 35 template releases in its own change log, so a generated type without it made every proto refresh a major version bump here. Downstream, struct expressions and `..Default::default()` stop working on these: use `RithmicConfigBuilder::from_env(env)`, `TrailingStop::new(..)`, or `Default::default()` followed by field assignment. Matches over generated enums need a `_` arm.
- **`RithmicOrder::price`, `RithmicOcoOrderLeg::price` and `RithmicModifyOrder::price` are now `Option<f64>`.** Wrap existing values in `Some(..)`; pass `None` for market orders, which previously shipped `price = 0.0`. A modify still restates the order — set `price` to the order's current price when only the quantity is changing — but an unset price is now omitted from the request and can no longer reach the wire as `0.0`, or stand in as a `0.0` trigger.
- **Order command types gain a `manual_or_auto` field** of the crate's own `ManualOrAutoEntry` enum, which defaults to `Auto`. The seven generated per-request-module `OrderPlacement` enums have no `Default` at all — a struct holding one by value cannot derive `Default` — so owning the enum, with `From` into each of them, is what lets the command types derive it.
...
- **`cancel_all_orders` is now attributed to `Auto` instead of `Manual`,** changing what the server records for origination. Build the command with `.manual_or_auto(ManualOrAutoEntry::Manual)` to keep the previous attribution.
...
- **`RithmicOrder::duration` is now a plain `TimeInForce` rather than `Option<TimeInForce>`,** matching `RithmicBracketOrder` and `RithmicOcoOrderLeg`. Unwrap existing values; `None` was sent as `Day`, which is what `TimeInForce::default()` is, so the wire is unchanged.
...
- **`RithmicOrderPlantHandle::show_fill_history(range, max_record_count)`** — the account's fill history (templates 3512/3513, new in template version 5.42), one response per fill. The new `FillHistoryRange` names the window in either index format the request takes: seconds since the epoch, or trade dates as CCYYMMDD. `max_record_count` above 10,000 is refused with `RithmicError::InvalidArgument`, since Rithmic rejects it.
- **`RithmicOrderPlantHandle::get_user_info(user)`** — a user's profile, entitlement status and session limits (templates 3510/3511); `None` asks about the logged-in user. The unsolicited **`RithmicMessage::UserInfoUpdate`** (template 357) carries user-level changes as they happen and previously arrived as `UnknownTemplate`.
- **`RithmicBracketOrder::operation_type`** and the `BracketOperationType` enum — the `order_operation_type` field Rithmic added to bracket requests in template version 5.37, selecting which event on one order of the bracket cancels the rest. Rithmic documents only the wire spellings (`AFOCCA`, `FOCCA`, `CCA`, `FCA`, `OCA`); each variant's rustdoc carries async_rithmic's reading of them (`FCA` = "fill cancels all", and so on), attributed as such. Unset, the field stays off the wire as before — async_rithmic tried defaulting it to `OCA` and reverted after it broke bracket orders.
- **The generated types pick up every field Rithmic added through template 5.42:** the contact, address, `order_copy_status` and per-plant session-count fields on `ResponseLoginInfo` (also on the new `ResponseGetUserInfo` and `UserInfoUpdate`); `loss_limit` and the account-creation timestamps on `ResponseAccountList`; `source_ssboe`/`source_usecs`/`source_nsecs` on `ExchangeOrderNotification`; `level_1_market_data`/`level_2_market_data` on `ResponseListExchangePermissions`, which Rithmic deprecates `entitlement_flag` in favor of; `min_qprice_change_precision` on `ResponseReferenceData`; `data_bar_seq_num` on `TickBar` and `ResponseTickBarReplay`; and `rms_updates_only` on `RequestPnLPositionUpdates`, which the PnL subscription leaves unset to keep streaming every update.
- **`validate()` on `RithmicOrder`, `RithmicBracketOrder`, `RithmicModifyOrder`, `RithmicOcoOrder` and `RithmicOcoOrderLeg`** — checks the command carries the prices its price type requires, returning `RithmicError::InvalidArgument`. The table is async_rithmic's: `Limit`, `StopLimit` and `LimitIfTouched` need a price, the stop and if-touched pairs need a trigger, `Market` needs nothing. On `RithmicModifyOrder` the price may stand in for the trigger, since moving a stop by its price alone predates `trigger_price`; on `RithmicBracketOrder` the exit legs are also checked as structure — quantities and tick distances pair up, every leg's quantity is positive, and a hand-set `bracket_type` matches the sides supplied. `build()` calls it; it is also public, so a command assembled another way can be checked. The other command types have no constraints to check and carry no `validate()`. Market rules beyond that stay unvalidated: Rithmic is the authority on what it accepts, and a stricter table compiled in here would refuse orders the server would have taken.
...
- **`RithmicModifyOrder::trigger_price`** — a modify can set a trigger distinct from the limit price. When `None`, the four triggering price types send `price` in its place.
- **`window_name` on `RithmicBracketOrder`, `RithmicModifyOrder`, `RithmicCancelOrder`, `RithmicExitPosition` and `RithmicOcoOrderLeg`** — the originating window Rithmic records, which `RithmicOrder` already carried. On the OCO leg it is per-leg, since `RequestOCOOrder` declares the field `repeated`; like `user_tag` it gets a slot per leg once any leg sets one, and is omitted when none does.
...
- **`RithmicModifyOrder::trail_by_ticks` and `if_touched`** — the last two `RequestModifyOrder` fields with no way to set them. `RequestModifyOrder` declares no `trail_by_price_id`, so the distance is a bare tick count rather than a `TrailingStop`, and the message's `trailing_stop` flag is derived from it.

  With these, every field of the eleven order request messages is now reachable from a command type. The request builders set each field explicitly rather than filling the remainder from `Default`, so a field added by a future proto refresh fails to compile instead of being silently dropped.
...
- **Command-type rustdoc examples are compiled rather than fenced ```ignore.** The old fences hid code that would not build.
- **`place_oco_order` no longer re-runs `validate()` on the group.** Validation belongs to `build()`, which is where the other place calls leave it; a leg the request template cannot express is still refused when the request is assembled. The leg-count check stays, since `validate()` never covered it.
...
- **A `MarketIfTouched` or `LimitIfTouched` modify omitted `trigger_price`.** The fallback to the order's own price covered only the two stop types, so an if-touched modify that named no separate level went out with the field unset. It now covers the same four types `validate()` demands a trigger for when the order is placed.
