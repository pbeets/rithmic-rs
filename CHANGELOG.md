...
- **`validate()` on `RithmicOrder`, `RithmicBracketOrder`, `RithmicOcoOrder` and `RithmicOcoOrderLeg`** — checks the command carries the prices its price type requires, returning `RithmicError::InvalidArgument`. `build()` calls it; it is also public, so a command assembled another way can be checked. The other command types have no constraints to check and carry no `validate()`. Nothing else is validated: Rithmic is the authority on what it accepts, and a stricter table compiled in here would refuse orders the server would have taken.
