...
- **`RithmicConfig`, `RithmicAccount`, `LoginConfig`, `InstrumentInfo`, `RithmicEnv`, `TrailingStop` and all 244 generated protobuf types are now `#[non_exhaustive]`.** Rithmic added fields to existing messages in 24 of the 35 template releases in its change log, so without this attribute every proto refresh would be a major version bump here. Downstream, struct literals and `..Default::default()` stop compiling on these types: use `RithmicConfigBuilder::from_env(env)`, `TrailingStop::new(..)`, or `Default::default()` followed by field assignment. Matches on generated enums need a `_` arm.
...
- **`TrailingStop` no longer implements `Default`** and gains a required `trail_by_price_id: i32`. Rithmic rejects a zero id with rp_code 1112, so `TrailingStop::default()` was guaranteed to be rejected. Use `TrailingStop::new(trail_by_ticks, trail_by_price_id)`. The convention across the crate: `new()` always returns `Self`; it takes no arguments on a type with a `Default`, and takes the required fields on a type without one.
...
- **`PartialEq` on `RithmicOrder`, `RithmicBracketOrder`, `RithmicOcoOrderLeg`, `RithmicModifyOrder`, `RithmicCancelOrder`, `RithmicIfTouchedTrigger` and `TrailingStop`** (and on the six new command structs above), so a built command can be compared in a test. `Eq` is not derived because of the `f64` price fields.
...
- **`place_oco_order` no longer re-runs `validate()` on the group.** Validation happens in `build()`, same as the other place calls. A leg the request template cannot express is still refused when the request is assembled, and the leg-count check stays — it was never part of `validate()`.
